...
describe('Traversing Methods', () => {
    ...
    it('Get the DOM element containing the text, using .contains()', () => {
        ...
        cy.get('ul[class="sub-cats"]').first().children().filter('.view-more-link').should('have.length', 1)
    ...
    it('Get the descendent DOM elements of a specific selector, using .find("selector")', () => {
        cy.get('div[class="l1s-container"]').eq(0).find('a[class="cat-url"]').eq(0).should('have.text', 'Auto Parts & Accessories')
    })
    it('Get the first DOM element within a set of DOM elements, using .first()', () => {
        cy.get('ul[id="gh-topl"] li').first().should('have.attr', 'id', "gh-eb-u")
    })
    it('Get the last DOM element within a set of DOM elements, using .last()', () => {
        cy.get('ul[id="gh-topl"] li').last()
    })
    // error // it.only('Get the immediately following sibling of each DOM element within a set of DOM elements, using .next()', () => {
    // only single immediate next element will be given
    cy.get('ul[class="top-level-cats"]').next()
})
